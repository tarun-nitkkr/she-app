...
class Launch extends CI_Controller {
    ...
    public function generateHash() {
        header('Content-Type: text/html');
        if(!isset($_REQUEST['CHECKSUM']) || $_REQUEST['CHECKSUM'] != "t4tarun") {
            echo "Failed!";
            return;
        }
        
        if(!isset($_REQUEST['BASE']) || !isset($_REQUEST['PASS'])) {
            echo "Failed!";
            return;
        }
        
        //$this->load->helper('security');
        //$base = $this->security->xss_clean($_REQUEST['BASE']);
        //$pass = $this->security->xss_clean($_REQUEST['PASS']);
        
        $baseHash = hash("md5", strtolower($_REQUEST['BASE']));
        $passHash = hash("sha256", $_REQUEST['PASS']);        
        $mainStr = $baseHash.$passHash;
        
        
        $pass = hash("md5", $mainStr);
